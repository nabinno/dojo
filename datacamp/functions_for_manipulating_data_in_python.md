...
## Using DATE_TRUNC
```sql
-- 1) Truncate rental_date by year
SELECT DATE_TRUNC('year', rental_date) AS rental_year
FROM rental;

-- 2) Truncate rental_date by month
SELECT DATE_TRUNC('month', rental_date) AS rental_month
FROM rental;

-- 3) Truncate rental_date by day of the month
...
## Getting into about user-defined data types
```sql
...
```sql0
-- 1) Select the film title and inventory ids
SELECT
    f.title,
    i.inventory_id
FROM film AS f
...
-- 2) Select the film title, rental and inventory ids
SELECT
    f.title,
    i.inventory_id,
...
-- 3) Select the film title and inventory ids
SELECT
    f.title,
    i.inventory_id,
    -- Determine whether the inventory is held by a customer
